...
def Dupplicate_count_column(dataf, ID_column, dup_column):
    ...
    dataf_dup = pd.DataFrame(data={ID_column: unique_keys,
...
def main(bedpath, output):
    bed_df = pd.read_csv(filepath_or_buffer=bedpath,
                         index_col=False, sep='\t', header=None, names=['chr', 'start', 'end', 'name', 'score', 'strand'])
    ID_column = 'ID'
    dup_column = 'quantity'
    bed_df[ID_column] = (bed_df['chr'].map(str))+'_'+(bed_df['start'].map(str)) + \
        '_'+(bed_df['end'].map(str))+'_'+(bed_df['strand'].map(str))
    ...
    bed_df = bed_df[cols]
    bed_df.to_csv(path_or_buf=output,
